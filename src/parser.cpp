...
TokenNode parseCond(Lexer &lex) {
  ...
  if (lex.peek(0).kind == TokenKind::LPAREN) {
    ...
    if (clauses.size() < 1) {
      throw std::runtime_error(
          "cond expressions must have at least one clause, found at line:" +
    ...
    while (!clauses.empty()) {
      if (size(clauses.top()) != 2) {
        throw std::runtime_error("cond clauses must have two and only two "
...
TokenNode parseLambda(Lexer &lex) {
  ...
  if (lex.peek(0).kind == TokenKind::LPAREN) {
    ...
    (void)lex.next(); // consume closing rparen
    return TokenNode{
        cons(TokenNode{lambda_}, cons(parameters, cons(body, TokenList{})))};
  }
  throw std::runtime_error("lambda must be followed by parameter list, found:" +
                           toString(lex.peek(0)));
}

// horrible state machine pattern <symbol colon <type or symbol> ...> arrow
// <type or symbol>
TokenNode validateParams(const TokenNode &params) {
  ...
  while (it != end) {
    ...
    if (isTokenNodeList(node)) { // if TokenList and state == 3 validate the
      ...
      case 4: {
        ...
        Token ret = Token(TokenKind::RETURN_TYPE, Value(ast), errtok.line,
                          errtok.column);
        ...
        it++;
        if (it != end) {
          throw std::runtime_error("param list did not terminate in a typelist "
                                   "or has more than one in lambda at line: " +
                                   toString(ret));
        }
        TokenList wrapped =
            cons(TokenNode{Token(TokenKind::PARAM_LIST, 0, 0)}, params_);
      ...
      default:
        ...
        oss << node;
        std::string msg =
            (state == 0 ? "param list must begin with a symbol, found:"
      ...
      case 0: // expect TokenKind::SYMBOL
        if (tok.kind != TokenKind::SYMBOL) {
          throw std::runtime_error(
              "param list must begin with a symbol, found:" + toString(tok));
        }
        state = 2;
      ...
      case 1: // expect TokenKind::SYMBOL or TokenKind::ARROW
        ...
        case TokenKind::ARROW:
          ...
          continue;
        default:
          throw std::runtime_error("expected argument or arrow, found:" +
                                   toString(tok));
        }
        break;
      case 2: // expect TokenKind::COLON
        if (tok.kind != TokenKind::COLON) {
          throw std::runtime_error("expected colon, found:" + toString(tok));
        }
        state++;
        it++;
        continue;
      ...
      case 4:
        ...
        Token ret =
            Token(TokenKind::RETURN_TYPE, Value(ast), tok.line, tok.column);
        TokenList params_ = cons(TokenNode{ret}, TokenList{});
        while (!arguments.empty()) {
          params_ = cons(TokenNode{arguments.top()}, params_);
          arguments.pop();
        ...
        it++;
        if (it != end) {
          throw std::runtime_error("param list did not terminate in a type or "
  ...
  throw std::runtime_error("invalid param list"); // idk how we get here but
                                                  // okay
}
  ...
  while (it != end) {
    ...
    case 0: // expect a type, a symbol or a TokenList
      if (isTokenNodeList(node)) {
        ...
        if (tok.kind == TokenKind::TYPE_IDENT) {
          ...
          typeNodes.push(TokenNode{
              Token(TokenKind::TYPE_VAR, *tok.value, tok.line, tok.column)});
          it++;
          state = 1;
          continue;
        } else {
          throw std::runtime_error("expected type in type list, found: " +
                                   toString(tok));
        }
      }
      break;
    ...
    case 2: // expect int
      if (isTokenNodeToken(node)) {
        ...
        throw std::runtime_error("expected number in composite type, found:" +
                                 toString(tok));
      }
      {
        std::ostringstream oss;
...
TokenNode unwrapIdent(Lexer &lex) {
  Token temp = lex.next();
  if (!temp.value) {
    throw std::runtime_error("attempted to unwrap identifier with no value: " +
                             toString(temp));
  }
...
TokenNode parseDefine(Lexer &lex) {
  ...
  case TokenKind::COLON: { // (define x:int expr) or (define x:(vec int 3) expr)
    ...
    (void)lex.next(); // consume closing rparen
    return TokenNode{
        cons(TokenNode{root}, cons(sym, cons(type, cons(expr, TokenList{}))))};
  }
    ...
    if (lex.peek(0).kind == TokenKind::COLON) {
      ...
      if (lex.peek(0).kind == TokenKind::TYPE_IDENT) {
        ...
        if (tok.kind == TokenKind::SYMBOL && tok.value) {
          ...
          throw std::runtime_error(
              "expected a type var in type position, found:" + toString(tok));
        }
      ...
      Token root =
          Token(TokenKind::LET_BINDING, Value(ast), lparen.line, lparen.column);
      if (lex.peek(0).kind == TokenKind::RPAREN) {
        (void)lex.next();
    ...
    if (name_.kind == TokenKind::SYMBOL) {
      if (lex.peek(0).kind == TokenKind::LPAREN) {
        ...
        throw std::runtime_error(
            ...
            " not followed with bindings list, found:" + toString(lex.peek(0)));
      }
    }
...
TokenNode validateTypeParams(const TokenNode &params) {
  ...
  while (it != end) {
    ...
    if (isTokenNodeToken(node)) {
      ...
      throw std::runtime_error(
          "expected type variable in parameters list for type lambda, found:" +
          toString(tok));
    }
    std::ostringstream oss;
    oss << node;
...
TokenNode parseTypeLambda(Lexer &lex) {
  ...
  if (lex.peek(0).kind == TokenKind::LPAREN) {
    ...
    if (lex.peek(0).kind != TokenKind::RPAREN) {
      throw std::runtime_error(
          "type lambda expressions may only have one body expression, found:" +
          toString(lex.peek(0)));
    }
    (void)lex.next();
    return TokenNode{
        cons(TokenNode{tlambda_}, cons(parameters, cons(body, TokenList{})))};
  }
  throw std::runtime_error(
...
TokenNode parseTypeApplication(Lexer &lex) {
  ...
  if (lex.peek(0).kind == TokenKind::RPAREN) {
    throw std::runtime_error("attempted to apply type lambda to no types in:" +
                             toString(tapply));
  }
...
TokenNode validateForall(const TokenNode &forall) {
  ...
  if (it != end) {
    ...
    oss << *it;
    throw std::runtime_error(
        "forall type expresssions may only have one body expression, found:" +
        oss.str());
  }
...
TokenList validateDottedList(const TokenList &lst) {
  ...
  if (dot_count == 1) {
    if (len < 3) {
      throw std::runtime_error("dotted lists cannot be of size < 3, at line:" +
                               std::to_string(line));
    }
    if (dot_pos != len - 1) {
      throw std::runtime_error(
  ...
  if (isTokenNodeList(ctor)) {
    ...
    if (size(lst) == 1 || size(lst) == 2) {
      ...
      if (size(lst) == 1) {
        ...
        return TokenNode{Token(TokenKind::CTOR_DECL, Value(decl), locTok.line,
                               locTok.column)};
      }
      ...
      auto decl = std::make_shared<AstNode>(TokenNode{ctorList});
      return TokenNode{
          Token(TokenKind::CTOR_DECL, Value(decl), locTok.line, locTok.column)};
    }
    throw std::runtime_error("constructor pattern list was of wrong arity:" +
